...
def test_normalize_us_phone_number():
    ...
    for phone_str, expected in test_cases:
        try:
            ...
            assert (
                normalized == expected
            ), f"Expected {expected} but got {normalized} for input {phone_str}"
...
def test_standardizeName():
    ...
    for name, expected in test_cases:
        ...
        assert (
            result == expected
        ), f"Expected {expected} but got {result} for input {name}"
...
def test_normalize_address():
    ...
    for addr, expected in test_cases:
        ...
        assert (
            result == expected
        ), f"Expected {expected} but got {result} for input {addr}"
...
def test_normalize_url():
    ...
    for url, expected in test_cases:
        ...
        assert (
            result == expected
        ), f"Expected {expected} but got {result} for input {url}"


def test_normalize_with_invalid_emails():
    emails_invalid = pd.DataFrame(
        {"email": ["raniaanjor#gmail.com", "Rania@.com", "@gmail.com"]}
    )
...
def test_normalize_with_valid_emails():
    emails_valid = pd.DataFrame(
        {
            "email": [
                "W3071442@aol.com",
                "Anjorinr1@student.iugb.edu.ci",
                "Raniaanjor@gmail.com",
            ]
        }
    )
    expected_output = pd.DataFrame(
