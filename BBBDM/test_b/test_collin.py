...
modular = importlib.util.spec_from_file_location("Collin", str(Path(__file__).parent.parent / 'data_processing/Collin.py'))

Collin = importlib.util.module_from_spec(modular)
modular.loader.exec_module(Collin)
...
def test_find_matches_pass():
    ...
    result_df = Collin.compare_dataframes(historicalData, newData)
